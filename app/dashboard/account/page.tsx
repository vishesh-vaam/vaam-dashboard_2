...
function AccountContent() {
  const session = useSession();
  const { supabase } = useSupabase();
  const { userProfile, loading: profileLoading } = useUserProfile(); // Use the shared hook
  const [profile, setProfile] = useState<Profile>({
    first_name: "",
    middle_name: "",
    last_name: "",
  ...
  useEffect(() => {
    ...
    // If we already have userProfile from the hook, use that
    if (userProfile) {
      const profileData = {
        ...
        car_model: userProfile.car_model || "",
        drivers_license_number: userProfile.drivers_license_number || "",
        insurance_file_url: userProfile.insurance_file_url || "",
      };
      ...
      // Fallback to fetching profile if hook hasn't loaded yet
      const fetchProfile = async () => {
        const { data, error } = await supabase
          .from("profiles")
          .select("*")
    ...
    setEditing(editMode); // Set editing mode based on URL parameter
  }, [session, supabase, editMode, userProfile]);
  ...
  // In your handleSave function in the account page
  const handleSave = async (e: FormEvent) => {
    e.preventDefault();
    if (!session?.user) return;
    setLoading(true);
    ...
    try {
      ...
      if (error) {
        ...
        // Hard reload the page to ensure all components are refreshed
